...
const UpdateEPostStatus = ({ onClose, rowData, form, setForm, setShowDocument }) => {
  ...
  const onFormValueChange = (setValue, formData, formState, reset, setError, clearErrors, trigger, getValues, index) => {
    if (JSON.stringify(formData.currentStatus) !== JSON.stringify(form?.currentStatus) || (formData?.dateOfDelivery && JSON.stringify(formData.dateOfDelivery) !== JSON.stringify(form?.dateOfDelivery))) {
    ...
    >
      <div style={{ padding: '16px' }}>
        <div style={{ marginBottom: '16px' }}>
          ...
          />
          {currentStatus === "Delivered" && <div>Date of Delivery : <span>{rowData?.original?.receivedDate}</span></div>}
          {currentStatus === "Not Delivered" && <div>Date of Delivery Attempted : <span>{rowData?.original?.receivedDate}</span></div>}
        </div>
        <div style={{ marginBottom: '16px', backgroundColor: '#f8f9fa', padding: '16px', borderRadius: '8px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
            ...
            <span style={clickableTextStyle} onClick={() => setShowDocument(true)}>{t("View Document")}</span>
